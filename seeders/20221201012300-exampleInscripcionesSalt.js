...
module.exports = {
  async up (queryInterface, Sequelize) {
    ...
    try{
      ...
      // Edo Mex
      const alumnosSalt = await sequelize.query('SELECT id FROM Alumnos WHERE campusId = 4',
      {
        type: sequelize.QueryTypes.SELECT
      });

      const seccionsEdo = await sequelize.query('SELECT id, periodoId  FROM Seccions WHERE campusId = 4',{
        type: sequelize.QueryTypes.SELECT
      });
